...
<xsl:stylesheet
    ...
    <xsl:template name="columns">
        ...
        <chunk-point/>
        <!-- Performance Optimization, see ChunkPointPlaceholderEventSource -->
        <div id="portalPageBodyColumns">
            <xsl:attribute
                    name="class"> <!-- Write appropriate FSS class based on number of columns to produce column layout. -->
            ...
            <xsl:for-each select="column">
                ...
                <xsl:variable name="COLUMN_WIDTH">
                    <xsl:choose>
                        ...
                        <xsl:otherwise>
                            <xsl:value-of select="floor(substring-before(@width,'%'))"/>
                        </xsl:otherwise>
                    </xsl:choose>
                </xsl:variable>
                ...
                <div id="column_{@ID}"
                     class="portal-page-column {$POSITION} fl-container-flex{$COLUMN_WIDTH} {$MOVABLE} {$DELETABLE} {$EDITABLE} {$CAN_ADD_CHILDREN}"> <!-- Unique column_ID needed for drag and drop. -->
                    <div id="inner-column_{@ID}"
                         class="portal-page-column-inner"> <!-- Column inner div for additional presentation/formatting options.  -->
                        <xsl:if test="$IS_FRAGMENT_ADMIN_MODE='true'">
                            <div class="column-permissions">
                                <a class="button portal-column-permissions-link" href="javascript:;">
                                    <span class="icon permissions"></span>
                                    <xsl:value-of
                                            select="upMsg:getMessage('edit.column.x.permissions', $USER_LANG, $NUMBER)"/>
                                </a>
                            </div>
                        </xsl:if>
                        <xsl:apply-templates select="channel|blocked-channel"/>
                        <!-- Render the column's portlets.  -->
                    </div>
                </div>
            </xsl:for-each>
        </div>
    ...
    <xsl:template name="sidebar">
        <div id="portalSidebar">
            <xsl:variable name="FSS_SIDEBAR_LOCATION_CLASS">
                <xsl:call-template name="sidebar.location"/>
                <!-- Template located in page.xsl. -->
            </xsl:variable>
            <xsl:variable name="SIDEBAR_USE_TOGGLE_CLASS">
                <xsl:choose>
                    <xsl:when test="$USE_SIDEBAR_TOGGLE='true' and $AUTHENTICATED='true'">sidebarToggle
                        <xsl:value-of select="$SIDEBAR_TOGGLE_INIT"/>
                    </xsl:when>
                    <xsl:otherwise></xsl:otherwise>
                </xsl:choose>
            </xsl:variable>
            <xsl:attribute name="class">
                <xsl:choose>
                    <xsl:when test="$AUTHENTICATED='true'">
                        <xsl:choose>
                            <!-- when focused -->
                            <xsl:when test="$PORTAL_VIEW='focused'">fl-col-fixed fl-force-<xsl:value-of
                                    select="$FSS_SIDEBAR_LOCATION_CLASS"/>
                                <xsl:value-of select="$SIDEBAR_USE_TOGGLE_CLASS"/>
                            </xsl:when>
                            <!-- when dashboard -->
                            <xsl:otherwise>fl-col-fixed fl-force-<xsl:value-of select="$FSS_SIDEBAR_LOCATION_CLASS"/>
                                <xsl:value-of select="$SIDEBAR_USE_TOGGLE_CLASS"/>
                            </xsl:otherwise>
                        </xsl:choose>
                    </xsl:when>
                ...
                <xsl:choose>
                    <xsl:when test="$AUTHENTICATED='true'">
                        <xsl:if test="$USE_SIDEBAR_TOGGLE='true'">
                            <div id="portalSidebarToggleButton" class="fl-widget">
                                <a hef="javascript:;"
                                   title="{upMsg:getMessage('sidebar.toggle.button.close', $USER_LANG)}">
                                    <h2 class="textopen">
                                        <xsl:value-of
                                                select="upMsg:getMessage('sidebar.toggle.button.open', $USER_LANG)"/>
                                    </h2>
                                    <h2 class="textclose">
                                        <xsl:value-of
                                                select="upMsg:getMessage('sidebar.toggle.button.close', $USER_LANG)"/>
                                    </h2>
                                </a>
                            </div>
                        </xsl:if>
                        <xsl:choose>
                            <xsl:when test="$PORTAL_VIEW='focused'">
                                ...
                                <xsl:if test="$USE_SIDEBAR_FOCUSED='true'">
                                    ...
                                    <xsl:call-template name="content.sidebar.focused.block"/>
                                    <!-- Calls a template of customizable content from universality.xsl. -->
                            ...
                            <xsl:otherwise>
                                ...
                                <xsl:if test="$USE_SIDEBAR='true'">
                                    <!-- ****** SIDEBAR BLOCK ****** -->
                                    <xsl:call-template name="content.sidebar.block"/>
                                    <!-- Calls a template of customizable content from universality.xsl. -->
                                    <!-- ****** SIDEBAR BLOCK ****** -->
                                </xsl:if>
                    ...
                    <xsl:otherwise>
                        ...
                        <xsl:if test="$USE_SIDEBAR_GUEST='true'">
                            ...
                            <xsl:call-template name="content.sidebar.guest.block"/>
                            <!-- Calls a template of customizable content from universality.xsl. -->
                            <!-- ****** CONTENT SIDEBAR GUEST BLOCK ****** -->
                        </xsl:if>
