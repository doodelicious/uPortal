                    ...
                    <tr style="text-transform:capitalize">
                        <c:forEach items="${ results[0] }" var="cell" varStatus="status">
                            <th id="${n}column${ status.index }" class="flc-pager-sort-header">
                                <a rsf:id="column${ status.index }" href="javascript:;">${ fn:escapeXml(cell.key) }</a>
                            </th>
                        </c:forEach>
                    </tr>
                    </thead>
...
<script type="text/javascript">
    up.jQuery(function () {
        ...
        var fluid = up.fluid;
        var results = [
                <c:forEach items="${ results }" var="row" varStatus="status">{<c:forEach items="${ row }" var="cell" varStatus="cellStatus">'column${ cellStatus.index }': '<spring:escapeBody htmlEscape="false" javaScriptEscape="true">${ cell.value }</spring:escapeBody>'${ cellStatus.last ? '' : ','}</c:forEach>}${ status.last ? '' : ','}</c:forEach>
        ];
