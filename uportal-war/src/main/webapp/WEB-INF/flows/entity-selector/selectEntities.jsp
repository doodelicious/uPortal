...
<div class="fl-widget-titlebar titlebar portlet-titlebar" role="sectionhead">
    ...
    <h3 class="subtitle"><spring:message code="${pageSubtitleCode}" arguments="${pageSubtitleArgument}"
                                         text="${pageSubtitleText}"/></h3>
</div>
<!--content-->
            ...
            <div class="fl-container-flex60">
                ...
                <div id="${n}entityBrowser" class="entity-browser">
                    ...
                    <div id="${n}entityBrowserTitlebar" class="titlebar ui-helper-clearfix">
                        ...
                        <a class="select" id="${n}selectEntityLink" href="javascript:;"
                           title="<spring:message code="select"/>"><span><spring:message code="select"/></span></a>
                    </div>
                    <!--content-->
                            ...
                            <c:choose>
                                <c:when test="${type == 'group'}">
                                    <div class="group">
                                        ...
                                        <p class="no-members" style="display:none"><spring:message
                                                code="no.member.subgroups"/></p>
                                    </div>
                                </c:when>
                                <c:when test="${type == 'person'}">
                                    <div class="person">
                                        ...
                                        <ul class="member-list"></ul>
                                        <p class="no-members" style="display:none"><spring:message
                                                code="no.direct.member.people"/></p>
                                    </div>
                                </c:when>
                                <c:when test="${type == 'category'}">
                                    <div class="category">
                                        ...
                                        <ul class="member-list"></ul>
                                        <p class="no-members" style="display:none"><spring:message
                                                code="no.member.subcategories"/></p>
                                    </div>
                                </c:when>
                                <c:when test="${type == 'portlet'}">
                                    <div class="portlet">
                                        ...
                                        <ul class="member-list"></ul>
                                        <p class="no-members" style="display:none"><spring:message
                                                code="no.direct.member.portlets"/></p>
                                    </div>
                                </c:when>
                            </c:choose>
                        </c:forEach>
                    ...
                    <div id="${n}portletSearch" class="portlet-search">
                        ...
                        <div id="${n}searchDropDown" class="search-dropdown">
                            ...
                            <div id="${n}searchResultsNoMembers" class="portlet-msg info" role="alert"><p>
                                <spring:message code="no.members"/></p></div>
                            ...
                            <div id="${n}selectionBasket" class="selection-basket">
                                <ul>
                                    <c:forEach items="${groups}" var="group">
                                        <li>
                                            <a key="${group.entityType}:${group.id}" href="javascript:;"><c:out
                                                    value="${group.name}"/></a>
                            ...
                            <div id="${n}buttonPanel" class="buttons">
                                <c:if test="${ showBackButton }">
                                    <input class="button" type="submit"
                                           value="<spring:message code="${ backButtonCode }" text="${ backButtonText }"/>"
                                           name="_eventId_back"/>
                                </c:if>
                                <input id="${n}buttonPrimary" class="button primary" type="submit"
                                       value="<spring:message code="${ saveButtonCode }" text="${ saveButtonText }"/>"
                                       name="_eventId_save"/>
                                <c:if test="${ showCancelButton }">
                                    <input class="button" type="submit"
                                           value="<spring:message code="${ cancelButtonCode }" text="${ cancelButtonText }"/>"
...
<c:otherwise>
    ...
    <div class="view-single-select">
                        ...
                        <c:choose>
                            <c:when test="${fn:length(groups) == 0}">
                                <span class="selection"
                                      title="<spring:message code="nothing.selected"/>"><spring:message
                                        code="nothing.selected"/></span>
                            </c:when>
                            <c:otherwise>
                                <c:forEach items="${groups}" var="group">
                                    <a key="${group.entityType}:${group.id}" href="javascript:;"
                                       class="selection"><c:out value="${group.name}"/></a>
                                    <input type="hidden" name="groups" value="${group.entityType}:${group.id}"/>
                                </c:forEach>
                            </c:otherwise>
                        </c:choose>
        ...
        <div id="${n}portletSearch" class="portlet-search">
            ...
            <div id="${n}searchDropDown" class="search-dropdown">
                <div id="${n}closeDropDown" class="search-close"><a href="javascript:;">Close</a></div>
                <div id="${n}searchResultsNoMembers" class="portlet-msg info" role="alert"><p><spring:message
                        code="no.members"/></p></div>
                <ul id="${n}searchResults" class="search-list">
                    <li class="group">
                        <a href="javascript:;" title="&nbsp;"><span>&nbsp;</span></a>
                    </li>
