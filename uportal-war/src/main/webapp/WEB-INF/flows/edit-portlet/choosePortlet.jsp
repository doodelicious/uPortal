...
<div class="fl-widget portlet ptl-mgr view-chooseportlet" role="section">
            ...
            <div class="buttons">
                <c:choose>
                    <c:when test="${ completed }">
                        <input class="button primary" type="submit" value="<spring:message code="review"/>"
                               name="_eventId_review"/>
                    </c:when>
                    <c:otherwise>
                        <input class="button primary" type="submit" value="<spring:message code="continue"/>"
                               name="_eventId_next"/>
    ...
    <script type="text/javascript">
        up.jQuery(function () {
            ...
            var portlets = {};
            <c:forEach items="${contexts}" var="context">
            portlets['${context.name}'] = [
                    <c:forEach items="${context.portlets}" var="portlet" varStatus="status">{name: '${portlet.portletName}', title: '${fn:length(portlet.displayNames) > 0 ? fn:replace(portlet.displayNames[0].displayName, "'", "\\'") : fn:replace(portlet.portletName, "'", "\\'")}'}${status.last ? '' : ','}</c:forEach>
            ];
            </c:forEach>
